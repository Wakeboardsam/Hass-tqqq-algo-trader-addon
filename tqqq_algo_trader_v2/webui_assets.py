...
def get_dashboard_html(symbol, price, pos, open_cost, closed_cost, reco_status, db_rows, tail_log, is_paused, season_stats):
    ...
    Now includes Season P/L and Last Season P/L cards.
    """
    ...
    for r in db_rows:
        ...
        row_style = ""
        if stat == "OPEN":
            row_style = "background-color: #e6ffe6; color: #006400;" 
        elif stat == "ORDER_SENT":
            row_style = "background-color: #fff9e6; color: #b38600;" 
        elif stat == "CLOSED":
            row_style = "background-color: #ffe6e6; color: #8b0000;" 
        elif stat == "PENDING":
            row_style = "color: #666;" 
    ...
    last_pl_color = "green" if season_stats['last_season_pl'] >= 0 else "red"
    ...
    <body>
        <div style="display: flex; justify-content: space-between; align-items: center;">
            ...
            <div><span style="font-size: 0.9em; color: #666;">Last Update: {reco_status.get('timestamp', '')}</span></div>
        </div>
        ...
        <div class="card-grid">
            <div class="card">
                <h4>Current Season P/L</h4>
                <p style="color: {pl_color};">${season_stats['current_pl']:,.2f}</p>
                <small>Started: ${season_stats['starting_equity']:,.2f}</small>
            </div>
            <div class="card">
                <h4>Last Season Result</h4>
                <p style="color: {last_pl_color};">${season_stats['last_season_pl']:,.2f}</p>
            </div>
            <div class="card">
                <h4>Current Equity</h4>
                <p>${season_stats['current_equity']:,.2f}</p>
            </div>
             <div class="card">
                <h4>Active Cash</h4>
                <p>${open_cost:,.2f}</p>
        ...
        <table>
            <thead><tr><th>Level</th><th>Status</th><th>Shares</th><th>Buy Target</th><th>Sell Target</th><th>Order ID</th></tr></thead>
